...
data = csvread('Obj1b_part2_gas.csv',1,0);
...
data_br = csvread('Obj1b_part2_br.csv',1,0);
...
C_o = 1; %dimensionless
t = 0:50:50500;   %default delta t is 25
...
figure (101)
hold on
n2oarea = cumtrapz(PV,C);
totn2oarea = trapz(PV,C);
totn2omass = (data_PV(end))*1;
normalizedn2o = n2oarea/totn2omass;
plot(PV, normalizedn2o)
% massbaln2o = normalizedn2o./normalizedbr;
% plot(PV, massbaln2o)


%Headspace Mass Balance
tracersoldata = csvread('Obj1b_part2_tracer_sol.csv',1,0);
n2otracer = tracersoldata(:,4);
tracertime = tracersoldata(:,1).*60;
tracerPV = v_m.*tracertime.*phi_m./L;
interptracer = interp1(tracerPV, n2otracer, PV);
...
headspacedata = csvread('Obj1b_part2_headspace_n2o.csv',1,0);
...
combinedheadspace = csvread('Obj1b_part2_headspace_n2o_combined.csv',1,0);
...
%Bubble Mass Balance
bubblen2o = normalizedbr-(normalizedn2o);%+normalizedheadspace);
figure(22)
plot(PV,bubblen2o)
bubblen2ochange = diff(bubblen2o);
find(bubblen2ochange == max(bubblen2ochange(:)))


%truncate data to 6.04 pore volumes
shortnormalizedn2o = normalizedn2o(1:869);
shortbubblen2o = bubblen2o(1:869);
shortnormalizedheadspace = normalizedheadspace(1:869);
shortPV = PV(1:869);

figure(222)
alln2o = [shortnormalizedn2o' shortnormalizedheadspace' shortbubblen2o'];
massbalpv = area(shortPV,alln2o)
...
ylim([0 1.2]);
xlim([0 8])
%title('N_2O Distribution Throughout Reactor', 'fontsize', 36);
%legend('Effluent', 'Bubble Entrapped', 'Evolved Headspace', 'Location', 'northwest');
set(gca, 'FontSize', 20);

figure(333)
finaleffluent = normalizedn2o(end);
finalbubble = 0;
...
predbubble = shortbubblen2o(end);
unantbubble = liberatedbubble - predbubble;
baralln2o = [finaleffluent, remainingheadspace, predbubble, unantbubble; nan(1,4)];
massbalbar = bar(baralln2o, 'stacked')
set(massbalbar,{'FaceColor'},{[0/255 128/255 255/255];[252/255 226/255 5/255];[255/255 40/255 0/255];'k'});
legend('Effluent', 'Mass Measured in Headspace Prior to Drainage', 'Mass Balance Estimate of Bubble Entrapped Mass', 'Additional Mass Released Following Drainage'); %
set(gca, 'FontSize', 20);
ylim([0 1.2]);
...
%Extend data to 7.04 PVs (Run for 6.04 PVs and add 1 PV for "effluent"
%remaining in reactor at experiment end
integralPV = PV(1:1011);
integralBrC = BrC(1:1011);
integraleff = C(1:1011);
