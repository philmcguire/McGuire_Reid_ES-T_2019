...
data = csvread('Obj2b_part2_gas.csv',1,0);
...
data_br = csvread('Obj2b_part2_br.csv',1,0);
...
dotime = xlsread('do_data_segment3_2bpart2.csv','A1:A100');
do_sec = dotime*60;
do_conc = xlsread('do_data_segment3_2bpart2.csv','B1:B100');
do_PV = v_m.*do_sec./L;
yyaxis right
...
%R_m = [1.01:0.05:1.41];
R_m = 1.19;
alpha = 1.92e-5;%2.97e-6;  %ethane 1.3e-4
%alpha = [1e-7:1e-7:20e-7];
...
yyaxis left
plot(PV, C,'-', 'Color', 'c', 'Linewidth', 2, 'HandleVisibility','off')
ylim([0 1.1])
xlim([0 3.25])
ylabel('C/C_o')
xlabel('PV')
...
%Shorten ch4C and interpolated_data vectors
intersect = intersections(PV, interpolated_data, PV, C)
PVindices = PV < intersect(4)
length(PV(PVindices));
shortPV = PV(1:length(PV(PVindices)));
shortC = C(1:length(PV(PVindices)));
shortinterpolated_data = interpolated_data(1:length(PV(PVindices)));

%Shade consumed ch4
transparency = 1
color2 = [128/255 255/255 0/255];
p5=jbfill(shortPV,shortC,shortinterpolated_data,color2,[128/255 255/255 0/255],0,transparency)
...
transparency = 1
earlyzerovec=zeros(length(shortinterpolated_data),1);
color3 = [0/255 0/255 128/255];
p6 = jbfill(shortPV,shortinterpolated_data,earlyzerovec',color3,[0/255 0/255 128/255],0.5,transparency)
hold on
latezerovec=zeros(length(interpolated_data)-length(shortinterpolated_data)+1,1);
latePV = PV(length(shortPV):(length(PV)));
lateC = C(length(shortPV):(length(C)));
jbfill(latePV,lateC,latezerovec',color3,[0/255 0/255 128/255],0.5,transparency)
hold on

plot(PV, C,'-', 'Color', 'c', 'Linewidth', 4, 'HandleVisibility','off')
p1 = plot(data_PV_br_plot, Br, '*k', 'MarkerSize', 18)
hold on
p2 = plot(data_PV, ch4, 'oc','MarkerFaceColor', 'c', 'MarkerSize', 18, 'MarkerEdgeColor', 'k')
hold on

lgd = legend([p1 p2 p3 p4 p5 p6],'Bromide','CH_4', 'DO', 'Bubble Entrapped CH_4', 'Consumed CH_4', 'Effluent Discharged CH_4'); %
lgd.FontSize = 14;
[h,icons,plots,legend_text] = legend([p1 p2 p3 p4 p5 p6],'Bromide','CH_4', 'DO', 'Bubble Entrapped CH_4', 'Consumed CH_4', 'Effluent Discharged CH_4'); %
icons(11).Children.MarkerSize = 20;
set(h, 'FontSize',8);
h.Position = [2.6712 0.5640 0.2671 0.3474];
set(gca, 'TickDir', 'out')

%Truncate data to 3.25 PVs
integralPV = PV(1:390);
integralBrC = BrC(1:390);
integraleff = C(1:390);
